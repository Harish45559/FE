...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (!token || !role) return setError("Login failed: invalid server response");
      ...
      if (errorData?.errors) setError(Object.values(errorData.errors)[0]);
      else setError(errorData?.message || "Login failed");
    }
  };
  ...
  return (
    <div className="login-page">
      ...
      <div className="login-card">
        ...
        {error && (
          <div className="alert error" id="login-error">{error}</div>
        )}
        ...
        <div className="customer-portal-link">
          <a href="/customer/login">Order Online as a Customer</a>
        </div>
      </div>
    </div>
  );
