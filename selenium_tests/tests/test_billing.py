import sys
import os
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..'))

import pytest
from selenium.webdriver.support import expected_conditions as EC
from pages.login_page import LoginPage
from pages.billing_page import BillingPage
from config import BASE_URL, ADMIN_USERNAME, ADMIN_PASSWORD, EMPLOYEE_USERNAME, EMPLOYEE_PASSWORD


def admin_login(driver, wait):
    LoginPage(driver, wait).open().login(ADMIN_USERNAME, ADMIN_PASSWORD)
    wait.until(EC.url_contains("/dashboard"))


def employee_login(driver, wait):
    LoginPage(driver, wait).open().login(EMPLOYEE_USERNAME, EMPLOYEE_PASSWORD)
    wait.until(EC.url_contains("/attendance"))


class TestBilling:

    def test_admin_can_access_billing(self, driver, wait):
        admin_login(driver, wait)
        page = BillingPage(driver, wait).open()
        assert page.is_loaded()

    def test_billing_shows_pos_layout(self, driver, wait):
        admin_login(driver, wait)
        page = BillingPage(driver, wait).open()
        assert page.is_pos_layout_visible()

    def test_billing_shows_search_input(self, driver, wait):
        admin_login(driver, wait)
        page = BillingPage(driver, wait).open()
        assert page.is_search_visible()

    def test_billing_shows_cart(self, driver, wait):
        admin_login(driver, wait)
        page = BillingPage(driver, wait).open()
        assert page.is_cart_visible()

    def test_employee_cannot_access_billing(self, driver, wait):
        employee_login(driver, wait)
        driver.get(f"{BASE_URL}/billing")
        wait.until(EC.url_contains("/attendance"))
        assert "/billing" not in driver.current_url

    def test_unauthenticated_cannot_access_billing(self, driver, wait):
        driver.get(f"{BASE_URL}/billing")
        wait.until(EC.url_contains("/login"))
        assert "/login" in driver.current_url
